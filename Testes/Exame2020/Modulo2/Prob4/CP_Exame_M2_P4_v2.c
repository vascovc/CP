...
int main (int argc, char *argv[]) {
	...
	*/
	MPI_Type_vector(3, N, N, MPI_DOUBLE, &filetype);
	MPI_Type_commit(&filetype);

	MPI_File_open(MPI_COMM_WORLD, "output_v2.bin", MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);

	/* int MPI_File_set_view(MPI_File fh, MPI_Offset disp, MPI_Datatype etype,
							 MPI_Datatype filetype, const char *datarep, MPI_Info info) */
	MPI_File_set_view(fh, N*sizeof(double)*myid, MPI_DOUBLE, filetype, "native", MPI_INFO_NULL);
