...
#define W (15.0/16.0) //para ser tratado como double - fator de relaxamento
...
int main(int argc, char *argv[])
{
    ...
    double (*Vnew)[ncols + 4]; // Numero de linhas da matriz Vnem. O +4 e por causa das fantasma
    double (*Vold)[ncols + 4];
    double (*myf)[ncols + 4];
    Vnew = calloc(nrows + 4, sizeof(*Vnew)); // O numero de linhas tem de ser igual ao numero de linhas +4 -> "fantastma"                                 
    Vold = calloc(nrows + 4, sizeof(*Vold));
    myf = calloc(nrows + 4, sizeof(*myf));

    double h = ((double)2 * L) / ((double) nx);
    // dominio nao inclui as primeiras e ultimas 2 linhas e colunas
    ...
    for (int iter = 0; iter < ITERMAX; iter++){
        ...
        if (sqrt(global_sums[0]/global_sums[1]) < TOL)
        {
            ...
            */
            ...
            MPI_File fp;
            MPI_File_open(comm2D, "results_2D_c.bin", MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fp);
        ...
        //alterado
        MPI_Sendrecv(Vnew[nrows], ncols+4, MPI_DOUBLE, nbrbottom, 0, Vnew[0] , ncols+4, MPI_DOUBLE, nbrtop, 0, comm2D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(Vnew[2], ncols+4, MPI_DOUBLE, nbrtop, 1, Vnew[nrows+2] , ncols+4, MPI_DOUBLE, nbrbottom, 1, comm2D, MPI_STATUS_IGNORE);

        MPI_Sendrecv(&(Vnew[0][ncols]), 1, column, nbrright, 2, &(Vnew[0][0]), 1, column, nbrleft, 2, comm2D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&(Vnew[0][2]), 1, column, nbrleft, 3, &(Vnew[0][ncols+2]), 1, column, nbrright, 3, comm2D, MPI_STATUS_IGNORE);
        
        //comunicacao das outras
        MPI_Sendrecv(Vnew[nrows+1], ncols+4, MPI_DOUBLE, nbrbottom, 4, Vnew[1] , ncols+4, MPI_DOUBLE, nbrtop, 4, comm2D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(Vnew[3], ncols+4, MPI_DOUBLE, nbrtop, 5, Vnew[nrows+3] , ncols+4, MPI_DOUBLE, nbrbottom, 5, comm2D, MPI_STATUS_IGNORE);

        MPI_Sendrecv(&(Vnew[0][ncols+1]), 1, column, nbrright, 6, &(Vnew[0][1]), 1, column, nbrleft, 6, comm2D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&(Vnew[0][3]), 1, column, nbrleft, 7, &(Vnew[0][ncols+3]), 1, column, nbrright, 7, comm2D, MPI_STATUS_IGNORE);
