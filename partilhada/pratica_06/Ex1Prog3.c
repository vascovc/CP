...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    if (myid == 0) {
      ...
      MPI_Ssend(bloco, n, MPI_INT, 1, 0, MPI_COMM_WORLD); //original
      // exercicio 1 -
      /*
      int buffer_size = (MPI_BSEND_OVERHEAD + n*sizeof(int));
      char* buffer = malloc(buffer_size);
      MPI_Buffer_attach(buffer, buffer_size);
      MPI_Bsend(bloco, n, MPI_INT, 1, 0, MPI_COMM_WORLD); // unica coisa a acrescentar e que parte
      */
      ///////
      //MPI_Send(bloco, n, MPI_INT, 1, 0, MPI_COMM_WORLD); // exer 2
      endtime=MPI_Wtime();
